...
/**
 * True on the public preview demo, where `PUBLIC_DEMO_MODE=true` is set in the
 * Vercel project env. Gates the preview chrome — the header PREVIEW chip, the
 * splash badge, and the free-tier-only guard on the tier selector. Unset (local
 * dev, or a private full-feature deployment) → false, so none of it shows.
 *
