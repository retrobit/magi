...
- **Temperament awareness** — Tells the synthesizer that each response came from a different dispositional lens, so it can surface _why_ perspectives diverge. The prompt describes this generically and asks the synthesizer to infer each node's lens from the actual response text — it does not hard-code the built-in persona names, so synthesis stays correct when temperaments are customized. **Synthesis only** — for Voting this toggle is greyed (with a tooltip), since voting has no single narrator to be "aware."
...
| `temperamentAwareness` | boolean | No       | Tell the synthesizer that each response came from a distinct dispositional lens so it can surface _why_ they diverge. The synthesizer infers each lens from the response text rather than relying on hard-coded persona names, so it stays correct when temperaments are customized. Defaults to `false`. |
